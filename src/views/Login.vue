<template>
        ...
        <!-- 登录框左侧 -->
          ...
          <div class="left-qrcode">
            <div id="qrcode" title="小鹿线">
              <canvas width="180" height="180" style="display: none"></canvas><img src="" style="display: block" />
            </div>
        ...
        <div class="login-right">
          <div class="login-form">
            <ul class="nav nav-tabs">
              <li class="nav-items" v-for='item in loginText' :key="item.id" 
              @click="loginChange(item.id)" :class='current==item.id?"actives":""'>
                ...
                <div class="tab-main">
                  <el-form ref="ruleFormRefPhone" :model="ruleFormPhone" :rules="rulesPhone">
                    ...
                    <el-form-item class="login-Verification" prop="captcha">
                      <el-input v-model='ruleFormPhone.captcha' placeholder="请输入验证码" />
                    ...
                    <div class="login-text">
                      同意相关服务条款和隐私政策 <a>《小鹿线用户服务协议》</a ><a>《小鹿线隐私政策》</a>
                      若您没有账号，系统将为您自动创建账号并登录。
                    </div>
                  </el-form>
                </div>
  ...
  <Verify ref="verify" :captcha-type="'blockPuzzle'"
   :img-size="{width:'400px',height:'200px'}"  @success="onSuccess">
  </Verify>
</template>
	    ...
	    if (valid) {
			loginByJson({
	      	...
	      	ElMessage({
                showClose:true,
                message:'请输入正确格式',
                type:'warconsole.log(res.data.accessToken);ning'
            })
	    }
	})
}
...
.login-right {
  ...
  width: 475px;
  height: 500px;
  background: url(../assets/images/ybbg.jpeg) no-repeat center center;
}
