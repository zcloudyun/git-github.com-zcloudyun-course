        ...
        <div class="main">
          ...
          <div v-if="cartList.length > 0">
            <ul class="haveorder"  v-for="(item,index) in cartList" :key=item.id>
              <li class="order-item">
                <el-checkbox v-model='item.check' @change='store.itemCheck(index)'></el-checkbox>
              </li>
...
import {cart,deleteCart,createToken} from '../api/index'
import { onBeforeMount } from '@vue/runtime-core'
import {user} from '../store/user'
import {storeToRefs} from 'pinia'
let router=useRouter()
let store=user()
let {cartList,isChecked,total} =storeToRefs(store)
onBeforeMount(()=>{
  cart().then(res=>{
    store.addCart(res.data.list)
  })
})
//点击全选
const checkAll=()=>{
