<template>
	<div class="main">
		...
		<div class='play'>
			<div class='play-left'>
        <vue3VideoPlay v-bind="options" @loadstart="onLoadstart" @timeupdate="onTimeupdate"/>
      </div>
...
import {play,recordHistory,getLastHistoryByChapterId} from '../api/index'
let route=useRoute()
...
const options=reactive({
  ...
  muted:false,//静音
  webFullScreen:false,
  speedRate:['0.75','1.0','1.25','1.5','2.0'],//播放速率
  autoPlay:false,//自动播放
  loop:false,//循环播放
  mirror:false,//镜像画面
  lightOff:false,//关灯模式
...
onBeforeMount(()=>{
    //获取播放课程数据
    play({courseId,chapterId}).then(res=>{
        //当前播放视频数据
...
const onLoadstart=(ev)=>{
  getLastHistoryByChapterId({
    memberId:user().userInfo.id,//会员ID
    courseId,//课程ID
    chapterId,//章节ID
  }).then(res=>{
