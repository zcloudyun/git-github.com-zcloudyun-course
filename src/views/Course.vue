  ...
  <div class="coursemain">
      ...
      <section class="search-container">
        <div class="search-item">
          ...
          <div class="all-items">
            ...
            <el-tag v-for="item in firstCategorys" :key="item.id" 
              :class="currentType['fcategory'].id == item.id? 'category-poniter'  : 'category-poniter-item'"
              effect="plain" type="info" @click="buildingCondition('fcategory', item.id)">
              {{ item.categoryName }}
            </el-tag>
          </div>
        </div>
        <div class="search-item search-item-transition" style="top: 45px">
          ...
          <div class="all-items">
            <el-tag class="category-poniter" effect="plain" type="info">全部</el-tag>
            <el-tag v-for="item in secondCategorys" :key="item.id"
              :class="currentType['scategory'].id == item.id ? 'category-poniter' : 'category-poniter-item'"
              effect="plain" type="info" @click="buildingCondition('scategory', item.id)">
              {{ item.categoryName }}
            </el-tag>
          </div>
        </div>
        <div class="search-item" style="top: 90px">
          ...
          <div class="all-items">
            <el-tag class="category-poniter" effect="plain" type="info">全部</el-tag>
            <el-tag v-for="item in courseLevel" :key="item.id"
              :class=" currentType['clevel'].id == item.code ? 'category-poniter':'category-poniter-item'"
              effect="plain" type="info" @click="buildingCondition('clevel', item.code)">
              {{ item.text }}
            </el-tag>
          </div>
        </div>
    ...
    <div class="main-container">
      <div class="container-top">
        <ul class="all">
          <li class="item" @click="handleZonghe" 
          :style='priceSortBy=="1"?"color:#2C80FF":""'>综合</li>
          <li class="item split">|</li>
          <li class="item" @click="handleNewCourse"
          :style='priceSortBy=="2"?"color:#2C80FF":""'>最新课程</li>
          <li class="item split">|</li>
          <li class="item" @click="mostbuy"
          :style='priceSortBy=="3"?"color:#2C80FF":""'>最多购买</li>
          <li class="item split">|</li>
          <li class="item-price" @click="handlePrice"
          :style='priceSortBy=="4"|| priceSortBy=="5"?"color:#2C80FF":""'>
            <span>价格</span>
            <span class="arrow">
